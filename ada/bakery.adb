...
-- Types, procedures and functions
  ...
  Choosing : array(0 .. Nr_Of_Processes - 1) of Integer range 0..1 := (others => 0);
  ...
  protected Max_Ticket_Tracker is 
    procedure Update_Max(Ticket : Integer);
  ...
  protected body Max_Ticket_Tracker is 
    ...
    procedure Update_Max(Ticket : Integer) is
      begin
        if Ticket > Max_Used then
          Max_Used := Ticket;
        end if;
    end Update_Max;
  ...
  begin
    ...
    Put_Line("MAX_TICKET=" & Trim(Integer'Image(Max_Ticket_Tracker.Get_Max), Ada.Strings.Left) & ";"); -- Place labels with extra info here (e.g. "MAX_TICKET=...;" for Backery). 
...
--          end loop;
          ...
          while Choosing(J) = 1 loop
            delay 0.001;
          end loop;

          while Number(J) /= 0 and then 
                (Number(Process.Id) > Number(J) or else
                  (Number(Process.Id) = Number(J) and Process.Id > J)) loop
            delay 0.001;
          end loop;
